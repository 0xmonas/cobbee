...
COMMENT ON FUNCTION "public"."admin_blacklist_wallet"("p_wallet_address" "text", "p_reason" "text", "p_notes" "text", "p_admin_wallet" "text", "p_ban_scope" "text") IS 'Blacklist supporter wallet (service role only)';
...
COMMENT ON FUNCTION "public"."admin_get_analytics"("p_days" integer) IS 'Get time-series analytics (service role only)';
...
COMMENT ON FUNCTION "public"."admin_search_users"("p_search_term" "text", "p_limit" integer) IS 'Search users with sensitive data (service role only)';
...
COMMENT ON FUNCTION "public"."admin_unblacklist_wallet"("p_wallet_address" "text", "p_admin_wallet" "text") IS 'Remove wallet from blacklist (service role only)';
...
CREATE TABLE IF NOT EXISTS "public"."supports" (
    ...
    CONSTRAINT "creator_reply_length" CHECK (("char_length"("creator_reply") <= 1000)),
...
COMMENT ON COLUMN "public"."supports"."creator_reply" IS 'Creator''s reply to the supporter''s message (max 1000 characters)';



COMMENT ON COLUMN "public"."supports"."creator_reply_at" IS 'Timestamp when creator replied to the message';
...
CREATE TABLE IF NOT EXISTS "public"."audit_logs" (
    ...
    CONSTRAINT "actor_type_values" CHECK (("actor_type" = ANY (ARRAY['user'::"text", 'admin'::"text", 'system'::"text", 'anonymous'::"text"])))
);
...
COMMENT ON CONSTRAINT "actor_type_values" ON "public"."audit_logs" IS 'Valid actor types: user (authenticated user), admin (platform admin), system (automated process), anonymous (unauthenticated actor)';
...
CREATE INDEX "idx_supports_creator_reply" ON "public"."supports" USING "btree" ("creator_id", "creator_reply_at" DESC) WHERE ("creator_reply" IS NOT NULL);
...
CREATE INDEX "idx_users_github_handle" ON "public"."users" USING "btree" ("github_handle") WHERE ("github_handle" IS NOT NULL);



CREATE INDEX "idx_users_instagram_handle" ON "public"."users" USING "btree" ("instagram_handle") WHERE ("instagram_handle" IS NOT NULL);
...
CREATE INDEX "idx_users_opensea_handle" ON "public"."users" USING "btree" ("opensea_handle") WHERE ("opensea_handle" IS NOT NULL);



CREATE INDEX "idx_users_tiktok_handle" ON "public"."users" USING "btree" ("tiktok_handle") WHERE ("tiktok_handle" IS NOT NULL);



CREATE INDEX "idx_users_twitter_handle" ON "public"."users" USING "btree" ("twitter_handle") WHERE ("twitter_handle" IS NOT NULL);
...
CREATE POLICY "Service role can insert supports directly" ON "public"."supports" FOR INSERT TO "service_role" WITH CHECK (true);



COMMENT ON POLICY "Service role can insert supports directly" ON "public"."supports" IS 'Allow server-side API (service role) to create support records via direct INSERT for x402 payment processing. Complements the existing RPC-based insert policy.';

...
/**
 * Verify OTP for Email Change (Custom Database Verification)
 *
 * POST /api/user/email/verify-change-otp
 *
 * Verifies the 6-digit OTP code for email change
 * Checks expiry and marks as used after successful verification
 * On success, updates email in public.users table
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!currentUser?.email) {
      return Response.json(
        { error: 'You must have an email before changing it.' },
        { status: 400 }
      )
    }
    ...
    // Now update public.users table with new email
